...
group_means <- aggregate(full_data[,!names(full_data) %in% 
                      c("activity_name","subject_num","activity_key","group")], 
                  by=list(full_data$subject_num, full_data$activity_name),
                  FUN=mean)


## rename group.1 and group.2 variables 
names(group_means)[names(group_means) %in% c("Group.1","Group.2")] <- c("subject_num", 
                                                                        "activity_name")
